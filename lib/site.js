...
export const site = {
  ...
  // Falls back to localhost so `npm run dev` works before you set any env vars.
  url: process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000',
  locale: 'en_US',
  twitter: '@clickvault', // >>> PLUG IN: your handle, or delete the twitter metadata block in app/layout.js
  contactEmail: 'hello@clickvault.example', // >>> PLUG IN: real inbox
  dmcaEmail: 'dmca@clickvault.example', // >>> PLUG IN: real inbox
}
 ...
 * See .env.local.example for what each one is.
 */
export const monetisation = {
  // "Claim Bonus" CTA under the game iframe.
  smartlinkUrl:
    process.env.NEXT_PUBLIC_SMARTLINK_URL || 'https://example.com/smartlink-placeholder',
